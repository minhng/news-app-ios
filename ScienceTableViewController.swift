...
class ScienceTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, ScienceTableViewCellDelegate {
    ...
    @IBOutlet var scienceTable: UITableView!
    var data: AnyObject?
    var jsonDetails: Any?
    var news = [News]()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
//        loadSampleNews()
//        scienceTable.delegate = self
//        scienceTable.dataSource = self
//        scienceTable.refreshControl = refresher
...
//
    ...
    func buttonTapped(cell: ScienceTableViewCell) {
        let index = self.scienceTable.indexPath(for: cell)!.section
        ...
        DispatchQueue.main.async(execute: self.scienceTable.reloadData)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "ScienceTableViewCell", for: indexPath) as? ScienceTableViewCell  else {
            fatalError("The dequeued cell is not an instance of ScienceTableViewCell.")
        }
        ...
        cell.scienceTitle.text = new.title
        if new.image.isEmpty {
            cell.scienceThumbnail.image = UIImage(named: "default-guardian")
        } else {
            let url = URL(string: new.image)
            cell.scienceThumbnail.kf.setImage(with: url)
        }
        cell.scienceSection.text = new.section
        ...
        if (hours != 0) {
            cell.scienceTime.text = String(Int(hours))+"h ago"
        } else if (minutes != 0) {
            cell.scienceTime.text = String(Int(minutes))+"m ago"
        } else {
            cell.scienceTime.text = String(Int(seconds))+"s ago"
        }
        if isBookmarked[indexPath.section]{
            cell.scienceBookmark.setImage(UIImage(systemName: "bookmark.fill"), for: .normal)
        } else {
            cell.scienceBookmark.setImage(UIImage(systemName: "bookmark"), for: .normal)
        }
        return cell
    }
    ...
    private func loadSampleNews() {
        ...
        Alamofire.request("https://minh-ios-backend-9.wl.r.appspot.com/guardianSection/science").responseJSON {
            response in
            SwiftSpinner.hide()
...
extension ScienceTableViewController: IndicatorInfoProvider {
