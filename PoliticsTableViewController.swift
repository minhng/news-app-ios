...
class PoliticsTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, PoliticsTableViewCellDelegate {
    ...
    @IBOutlet var politicsTable: UITableView!
    var data: AnyObject?
    var jsonDetails: Any?
    var news = [News]()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
//        loadSampleNews()
//        politicsTable.delegate = self
//        politicsTable.dataSource = self
//        politicsTable.refreshControl = refresher
...
//
    ...
    func buttonTapped(cell: PoliticsTableViewCell) {
        let index = self.politicsTable.indexPath(for: cell)!.section
        ...
        DispatchQueue.main.async(execute: self.politicsTable.reloadData)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "PoliticsTableViewCell", for: indexPath) as? PoliticsTableViewCell  else {
            fatalError("The dequeued cell is not an instance of PoliticsTableViewCell.")
        }
        ...
        cell.politicsTitle.text = new.title
        if new.image.isEmpty {
            cell.politicsThumbnail.image = UIImage(named: "default-guardian")
        } else {
            let url = URL(string: new.image)
            cell.politicsThumbnail.kf.setImage(with: url)
        }
        cell.politicsSection.text = new.section
        ...
        if (hours != 0) {
            cell.politicsTime.text = String(Int(hours))+"h ago"
        } else if (minutes != 0) {
            cell.politicsTime.text = String(Int(minutes))+"m ago"
        } else {
            cell.politicsTime.text = String(Int(seconds))+"s ago"
        }
        if isBookmarked[indexPath.section]{
            cell.politicsBookmark.setImage(UIImage(systemName: "bookmark.fill"), for: .normal)
        } else {
            cell.politicsBookmark.setImage(UIImage(systemName: "bookmark"), for: .normal)
        }
        return cell
    }
    ...
    private func loadSampleNews() {
        ...
        Alamofire.request("https://minh-ios-backend-9.wl.r.appspot.com/guardianSection/politics").responseJSON {
            response in
            SwiftSpinner.hide()
                ...
                let jsonResponse = JSON(responseStr)["response"]["results"].arrayValue
                    for result in jsonResponse {
                        ...
                        guard let new = News(title: newsTitle, image: newsThumbnail, section: newsSection, date: newsDate, id: newsId) else {
                            fatalError("Unable to instantiate politics")
                        }
                        var tempFlag = false
                        for i in self.news {
                            if i.id == newsId {
...
extension PoliticsTableViewController: IndicatorInfoProvider {
