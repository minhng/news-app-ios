...
class WorldTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, WorldTableViewCellDelegate {
    ...
    @IBOutlet var worldTable: UITableView!
    var data: AnyObject?
    var jsonDetails: Any?
    var news = [News]()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "WorldTableViewCell", for: indexPath) as? WorldTableViewCell  else {
            fatalError("The dequeued cell is not an instance of WorldTableViewCell.")
        }
        ...
        cell.worldTitle.text = new.title
        if new.image.isEmpty {
            cell.worldThumbnail.image = UIImage(named: "default-guardian")
        } else {
            let url = URL(string: new.image)
            cell.worldThumbnail.kf.setImage(with: url)
        }
        cell.worldSection.text = new.section
        ...
        if (hours != 0) {
            cell.worldTime.text = String(Int(hours))+"h ago"
        } else if (minutes != 0) {
            cell.worldTime.text = String(Int(minutes))+"m ago"
        } else {
            cell.worldTime.text = String(Int(seconds))+"s ago"
        }
        if isBookmarked[indexPath.section]{
            cell.worldBookmark.setImage(UIImage(systemName: "bookmark.fill"), for: .normal)
        } else {
            cell.worldBookmark.setImage(UIImage(systemName: "bookmark"), for: .normal)
        }
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        Alamofire.request("https://minh-ios-backend-9.wl.r.appspot.com/article", parameters: params).responseJSON {
            ...
            case .success: do {
                ...
                SwiftSpinner.hide()
                DispatchQueue.main.async(execute: self.worldTable.reloadData)
            }
...
//        worldTable.refreshControl = refresher
...
////        let placeId = cellResult["place_id"] as! String
        self.view.superview?.makeToast("Article bookmarked. Check out the Bookmarks tab to view", duration: 3.0, position: .bottom)
...
//
    func removeFavorite(cellResult: News) {
        let news_id = cellResult.id
        self.view.superview?.makeToast("Article removed from Bookmarks", duration: 3.0, position: .bottom)
        self.bookmarkStored?.removeValue(forKey: news_id)
        userDefault.set(self.bookmarkStored, forKey: "bookmark")
    }
    
    func buttonTapped(cell: WorldTableViewCell) {
//        print("index: ", self.homeTable.indexPath(for: cell))
        let index = self.worldTable.indexPath(for: cell)!.section
    ...
    private func loadSampleNews() {
        ...
        Alamofire.request("https://minh-ios-backend-9.wl.r.appspot.com/guardianSection/world").responseJSON {
            response in
            SwiftSpinner.hide()
                ...
                let jsonResponse = JSON(responseStr)["response"]["results"].arrayValue
                    for result in jsonResponse {
                        ...
                        guard let new = News(title: newsTitle, image: newsThumbnail, section: newsSection, date: newsDate, id: newsId) else {
                            fatalError("Unable to instantiate world")
                        }
                        var tempFlag = false
                        for i in self.news {
                            if i.id == newsId {
...
extension WorldTableViewController: IndicatorInfoProvider {
