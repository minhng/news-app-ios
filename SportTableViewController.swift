...
class SportTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, SportTableViewCellDelegate {
...
//        sportTable.refreshControl = refresher
...
//
    ...
    func buttonTapped(cell: SportTableViewCell) {
        let index = self.sportTable.indexPath(for: cell)!.section
        ...
        DispatchQueue.main.async(execute: self.sportTable.reloadData)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "SportTableViewCell", for: indexPath) as? SportTableViewCell  else {
            fatalError("The dequeued cell is not an instance of SportTableViewCell.")
        }
        ...
        cell.sportTitle.text = new.title
        if new.image.isEmpty {
            cell.sportThumbnail.image = UIImage(named: "default-guardian")
        } else {
            let url = URL(string: new.image)
            cell.sportThumbnail.kf.setImage(with: url)
        }
        cell.sportSection.text = new.section
        ...
        if (hours != 0) {
            cell.sportTime.text = String(Int(hours))+"h ago"
        } else if (minutes != 0) {
            cell.sportTime.text = String(Int(minutes))+"m ago"
        } else {
            cell.sportTime.text = String(Int(seconds))+"s ago"
        }
        if isBookmarked[indexPath.section]{
            cell.sportBookmark.setImage(UIImage(systemName: "bookmark.fill"), for: .normal)
        } else {
            cell.sportBookmark.setImage(UIImage(systemName: "bookmark"), for: .normal)
        }
        return cell
    }
    ...
    private func loadSampleNews() {
        ...
        Alamofire.request("https://minh-ios-backend-9.wl.r.appspot.com/guardianSection/sport").responseJSON {
            response in
            SwiftSpinner.hide()
                ...
                let jsonResponse = JSON(responseStr)["response"]["results"].arrayValue
                    for result in jsonResponse {
                        ...
                        guard let new = News(title: newsTitle, image: newsThumbnail, section: newsSection, date: newsDate, id: newsId) else {
                            fatalError("Unable to instantiate sports")
                        }
                        var tempFlag = false
                        for i in self.news {
                            if i.id == newsId {
...
extension SportTableViewController: IndicatorInfoProvider {
